...
const BRAND_RULES = {
  ...
  incorrect: [
    ...
    'combatclutter.com',    // domain (acceptable in URLs only)
  ],
  contexts: {
...
const IGNORE_PATTERNS = [
  ...
  'build',
  '.git',
  'combat-clutter-spec.json', // Ignore spec file
];
...
function scanFile(filePath) {
  ...
  lines.forEach((line, lineNumber) => {
    BRAND_RULES.incorrect.forEach(incorrectTerm => {
      // Skip if it's a URL context (domain is acceptable)
      if (incorrectTerm === 'combatclutter.com' && 
          (line.includes('http') || line.includes('href') || line.includes('url'))) {
        return;
      }
